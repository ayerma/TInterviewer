...
for (const file of entryFiles) {
    const entry = JSON.parse(fs.readFileSync(path.join(ENTRIES_DIR, file), 'utf-8'));
    const { spaceId, topicId, question } = entry;

    const space = index.spaces.find(s => s.id === spaceId);
    if (!space) {
        console.warn(`[WARN] Space "${spaceId}" not found in spaces-index.json — skipping ${file}`);
        continue;
    }

    const topic = space.topics.find(t => t.id === topicId);
    if (!topic) {
        console.warn(`[WARN] Topic "${topicId}" not found in space "${spaceId}" — skipping ${file}`);
        continue;
    }
